...
model_plot_A <- ggplot(abatement_costs, aes(x = Abatement)) +
  geom_smooth(aes(y = MC_A), method = lm, se = TRUE, size = 0.5, color = "black") +
  geom_point(aes(y = MC_A, color = "MC_A")) +
  geom_smooth(aes(y = MC_B), method = lm, se = TRUE, size = 0.5, color = "black") +
  geom_point(aes(y = MC_B, color = "MC_B")) +
  geom_smooth(aes(y = MC_C), method = lm, se = TRUE, size = 0.5, color = "black") +
  geom_point(aes(y = MC_C, color = "MC_C")) +
  geom_smooth(aes(y = MC_D), method = lm, se = TRUE, size = 0.5, color = "black") +
...
Equations: 
Sector A: cost_abatement = 0.57684X  - 8.64448; A = 180 
Sector A demand: demand = 103.8312 - 0.57684X 
In sector A, demand at 180 will be 0, as firms are producing at 180 with no regulations in place. Using this and the slope of the abatement cost curve, we caclulated the intercept of the demand curve

Sector B: cost_abatement = 9.31770 + 0.19874X; B = 200
Sector C: 

Graph of demand curves 
```{r}
...
demand_curves <- ggplot(tidy_costs, aes(x = Abatement, y = Cost)) +
  stat_function(fun = demand_function_A, aes(color = "A")) +
  stat_function(fun = demand_function_B, aes(color = "B")) +
  stat_function(fun = demand_function_C, aes(color = "C")) +
  stat_function(fun = demand_function_D, aes(color = "D")) +
  theme_classic()
